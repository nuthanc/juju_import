...
def prepare_config_file():
    os.system(
        "juju show-controller > /root/juju_import/cc-charm-2005-auto/controller.yml")

    with open('/root/juju_import/cc-charm-2005-auto/controller.yml') as file:
        controller = yaml.load(file, Loader=yaml.FullLoader)
        juju_config = {}
        cert = controller['juju-controller']['details']['ca-cert']
        cert_bytes = cert.encode('ascii')
        base64_bytes = base64.b64encode(cert_bytes)
        juju_config['juju-ca-cert'] = base64_bytes.decode('ascii')
        juju_config['juju-controller'] = controller['juju-controller']['details']['api-endpoints'][0].split(":")[
            0]
        juju_config['juju-model-id'] = controller['juju-controller']['models']['contrail']['model-uuid']
...
def wait_till_machine_is_deployed():
    juju_status = subprocess.run(
        ['juju', 'status'], stdout=subprocess.PIPE, universal_newlines=True)
...
def run_action_config(deploy_unit):
    out = subprocess.run(['juju', 'run-action', deploy_unit, 'import-cluster',
                          '--params', 'config.yaml'], stdout=subprocess.PIPE, universal_newlines=True)
    print(out.stdout)
...
def action_status_and_result(id):
    ...
    action_status = subprocess.run(
        ['juju', 'show-action-status', id], stdout=subprocess.PIPE, universal_newlines=True)
    ...
    action_result = subprocess.run(
        ['juju', 'show-action-output', id], stdout=subprocess.PIPE, universal_newlines=True)
...
def parse_deploy_unit():
    deploy_unit_output = subprocess.check_output(
        'juju status|grep "contrail-command/"', shell=True, universal_newlines=True)
...
def deploy(charm_path='/root/tf-charms/contrail-command', image_tag='2005.1'):
    ...
    juju_deploy = subprocess.run(['juju', 'deploy', charm_path, '--constraints', 'tags=contrail-command', '--config', 'docker-registry=svl-artifactory.juniper.net/contrail-nightly',
                                  '--config', image_tag_config, '--config', 'docker-registry-insecure=true'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
